...
class Transformer(nn.Module):
    ...
    def generate_mask(self, src, tgt):
        ...
        """
        # Source Mask: Mask padding tokens (1 where valid, 0 where padding)
        src_mask = (src != 0).unsqueeze(1).unsqueeze(2).to(src.device)   # (batch, 1, 1, src_len)

        # Target Mask: Mask padding tokens
        tgt_mask = (tgt != 0).unsqueeze(1).unsqueeze(3).to(tgt.device)  # (batch, 1, tgt_len, 1)

        # Causal Mask (No-Peek Mask): Prevents attending to future tokens
        seq_length = tgt.size(1)
        nopeak_mask = (1 - torch.triu(torch.ones(1, seq_length, seq_length), diagonal=1)).bool()  # (1, tgt_len, tgt_len)
        nopeak_mask = nopeak_mask.to(tgt.device)  # This is the fix

        # Apply padding + causal mask
